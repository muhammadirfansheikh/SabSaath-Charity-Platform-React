...
import Modal_PaymentType from "components/modal/ModalPaymentType.js";
import Swal from "sweetalert2";

const PaymentType = (props) => {
    const initialValues = {
        searchPaymentTypeName: "",
    ...
    const resetFormelement = async() =>{
        ...
        ReBindGrid(SetupMasterIds.PaymentType,0,"",0);
    ...
    const onDelete =async ({ PaymentTypeId }) => {
       ...
       var deleteData= await DeleteSetupDetail(PaymentTypeId,UserId,UserIp);
    ...
    const handleSearchClick=async(e) =>{
        ...
        ReBindGrid(SetupMasterIds.PaymentType,"0",values.searchPaymentTypeName,0);
                    ...
                    <Col md={3}>
                                            <FormGroup>
                                                ...
                                                <Input
                                                    ...
                                                    name="searchPaymentTypeName"
                                                    value={values.searchPaymentTypeName}
                                                    maxLength="50"
                                                    isalphabetic="true"
                                                    autoComplete="off"
                                                />
                            ...
                            <CardHeader>
                                <Row>
                                    ...
                                    <Col lg={6} md={6} className="text-right">
                                        <Button color="primary2" size="sm" className="m-0" onClick={() => openNewmodal({ PaymentTypeId: 0 })}>Add New</Button>
                                    </Col>
                                </Row>
                            </CardHeader>
                            <CardBody>
                                        ...
                                        {paymentTypeList && paymentTypeList.map((item, key) => (
                                            <tr key={key}>
                                                ...
                                                <td className="text-center">
                                                    <Button color="primary" className="btn-circle" size="sm" onClick={(e) => onEdit({ PaymentTypeId: item.SetupDetailId })}><i className="nc-icon nc-ruler-pencil"></i></Button>
                                                    <Button color="danger" className="btn-circle" size="sm" onClick={() => onDelete({ PaymentTypeId: item.SetupDetailId })}><i className="nc-icon nc-simple-remove"></i></Button>
                                                </td>
                                            </tr>
                                        ))}
                                    </tbody>
