...
import ModalExpense from '../components/modal/ModalExpense.js'
...
const Expense = (props) => {
    ...
    React.useEffect(() => {
        ...
        const load = async () => {
            ReBindGrid(SetupMasterIds.Expense,0,"",0);
        };
        load();
    }, []);
    ...
    const onDelete =async ({ ExpenseId }) => {
       ...
       var deleteData= await DeleteSetupDetail(ExpenseId,UserId,UserIp);
    ...
    const handleSearchClick=async(e) =>{
        ...
        ReBindGrid(SetupMasterIds.Expense,0,expensevalues,0);
                                    ...
                                    <Row form>
                                        <Col md={3}>
                                            <FormGroup>
                                                ...
                                                <Input
                                                    type="text"
                                                    onChange={(e) => setexpensevalues(AllowAlphabatic(e.target.value))}
                                                    name="expensename"
                                                    autoComplete="off"
                                                    maxLength="50"
                                                    value={expensevalues}
                            ...
                            <CardHeader>
                                <Row>
                                    ...
                                    <Col lg={6} md={6} className="text-right">
                                        <Button color="primary2" size="sm" className="m-0" onClick={() => openNewmodal({ ExpenseId: 0 })}>Add New</Button>
                                    </Col>
                                </Row>
                            </CardHeader>
                            <CardBody>
                                <Table bordered striped>
                                    ...
                                    <tbody>
                                        {expenseList && expenseList.map((item, key) => (
                                            <tr key={key}>
                                                <td>{key + 1}</td>
                                                ...
                                                <td className="text-center">
                                                    <Button color="primary" className="btn-circle" size="sm" onClick={(e) => onEdit({ ExpenseId: item.SetupDetailId })}><i className="nc-icon nc-ruler-pencil"></i></Button>
                                                    <Button color="danger" className="btn-circle" size="sm" onClick={() => onDelete({ ExpenseId: item.SetupDetailId })}><i className="nc-icon nc-simple-remove"></i></Button>
                                                </td>
                                            </tr>
                                        ))}
                                    </tbody>
