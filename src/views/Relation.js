...
import ModalRelation from '../components/modal/ModalRelation.js'
...
const Relation = (props) => {
    ...
    const [relationEditId, setrelationEditId] = useState(0);
    ...
    React.useEffect(() => {
        ...
        const load = async () => {
            ReBindGrid(SetupMasterIds.Relation,0,"",0);
        };
        load();
    }, []);

    const onEdit = ({ RelationId }) => {
     
        openNewmodal(RelationId);
    }

    const onDelete =async ({ RelationId }) => {
       
        var UserId = localStorage.getItem('UserId');
        var UserIp = localStorage.getItem('UserIP');
  
       var deleteData= await DeleteSetupDetail(RelationId,UserId,UserIp);
    ...
    const handleSearchClick=async(e) =>{
        ...
        ReBindGrid(SetupMasterIds.Relation,0,relationvalues,0);
    ...
    const openNewmodal = (RelationId) => {
       
        setrelationEditId(RelationId);
        setOpenModal(true);
    }
    const closeNewmodal = () => {
        setOpenModal(false);
                                    ...
                                    <Row form>
                                        <Col md={3}>
                                            <FormGroup>
                                                ...
                                                <Input
                                                    type="text"
                                                    onChange={(e) => setrelationvalues(AllowAlphabatic(e.target.value))}
                                                    name="relationname"
                                                    autoComplete="off"
                                                    maxLength="50"
                            ...
                            <CardHeader>
                                <Row>
                                    ...
                                    <Col lg={6} md={6} className="text-right">
                                        <Button color="primary2" size="sm" className="m-0" onClick={() => openNewmodal({ RelationId: 0 })}>Add New</Button>
                                    </Col>
                                </Row>
                            </CardHeader>
                            <CardBody>
                                <Table bordered striped>
                                    ...
                                    <tbody>
                                        {relationList && relationList.map((item, key) => (
                                            <tr key={key}>
                                                <td>{key + 1}</td>
                                                ...
                                                <td className="text-center">
                                                    <Button color="primary" className="btn-circle" size="sm" onClick={(e) => onEdit({ RelationId: item.SetupDetailId })}><i className="nc-icon nc-ruler-pencil"></i></Button>
                                                    <Button color="danger" className="btn-circle" size="sm" onClick={() => onDelete({ RelationId: item.SetupDetailId })}><i className="nc-icon nc-simple-remove"></i></Button>
                                                </td>
                                            </tr>
                                        ))}
                                    </tbody>
