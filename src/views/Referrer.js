...
import ModalReferrer from '../components/modal/ModalReferrer.js'
...
const Referrer = (props) => {
    ...
    const [referrerEditId, setreferrerEditId] = useState(0);
    ...
    React.useEffect(() => {
        ...
        const load = async () => {
            ReBindGrid(SetupMasterIds.Referrer,0,"",0);
        };
        load();
    }, []);

    const onEdit = ({ ReferrerId }) => {
      
        openNewmodal(ReferrerId);
    }

    const onDelete =async ({ ReferrerId }) => {
       
        var UserId = localStorage.getItem('UserId');
        var UserIp = localStorage.getItem('UserIP');
  
       var deleteData= await DeleteSetupDetail(ReferrerId,UserId,UserIp);
    ...
    const handleSearchClick=async(e) =>{
        ...
        ReBindGrid(SetupMasterIds.Referrer,0,referrervalues,0);
    ...
    const ReBindGrid =async (setupMasterId,parentId,setupDetailName,setupDetailId = 0) =>{
        ...
        setreferrerList(data.data);

    }
    const openNewmodal = (ReferrerId) => {
      
        setreferrerEditId(ReferrerId);
        setOpenModal(true);
    }
    const closeNewmodal = () => {
        setOpenModal(false);
        ...
        <>
            <div className="content">
                                    ...
                                    <Row form>
                                        <Col md={3}>
                                            <FormGroup>
                                                ...
                                                <Input
                                                    type="text"
                                                    onChange={(e) => setreferrervalues(AllowAlphabatic(e.target.value))}
                                                    name="referrername"
                                                    value={referrervalues}
                                                    autoComplete="off"
                                                    maxLength="50"
                                                />
                                            </FormGroup>
                            ...
                            <CardHeader>
                                <Row>
                                    ...
                                    <Col lg={6} md={6} className="text-right">
                                        <Button color="primary2" size="sm" className="m-0" onClick={() => openNewmodal({ ReferrerId: 0 })}>Add New</Button>
                                    </Col>
                                </Row>
                            </CardHeader>
                            <CardBody>
                                <Table bordered striped>
                                    ...
                                    <tbody>
                                        {referrerList && referrerList.map((item, key) => (
                                            <tr key={key}>
                                                <td>{key + 1}</td>
                                                ...
                                                <td className="text-center">
                                                    <Button color="primary" className="btn-circle" size="sm" onClick={(e) => onEdit({ ReferrerId: item.SetupDetailId })}><i className="nc-icon nc-ruler-pencil"></i></Button>
                                                    <Button color="danger" className="btn-circle" size="sm" onClick={() => onDelete({ ReferrerId: item.SetupDetailId })}><i className="nc-icon nc-simple-remove"></i></Button>
                                                </td>
                                            </tr>
                                        ))}
                                    </tbody>
                ...
                </Row>
                {
                    openModal &&
                    <ModalReferrer {...props}
                        HeaderText="Add/Edit Referrer"
                        Ismodalshow={openModal}
                        closeNewmodal={closeNewmodal}
                        ReferrerId={referrerEditId}
